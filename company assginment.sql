...
SELECT* FROM DEPARTMENT
 ...
 SELECT D.DEP_NAME, AVG(P.RATING) AS AVG_RATING FROM EMPLOYEE E 
...
SELECT D.DEP_NAME, P.RATING FROM DEPARTMENT D 
JOIN EMPLOYEE E ON D.DEP_ID = E.DEP_ID 
JOIN PERFORMANCE P ON E.EMP_ID = P.PERF_ID where P.RATING >3.5
GROUP BY D.DEP_NAME, P.RATING
...
CREATE OR ALTER TRIGGER TR_SALARY_UP
ON EMPLOYEE
AFTER UPDATE
AS
BEGIN
...
DECLARE  @NAME NVARCHAR(20),@SAL DECIMAL(10,2),@BONUS INT
